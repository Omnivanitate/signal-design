...
from signal_design.relation import Relation
from signal_design.axis import Axis
from signal_design.help_types import Number
from signal_design.exc import NotEqualError, BadInputError, TypeFuncError
...
class WrapperTestRelation:
    class TestBaseRelation(unittest.TestCase):
        def setUp(self) -> None:
            ...
            self.x_axis = Axis.get_using_end(start=0.0, end=0.5, sample=0.1)
            self.relation = self.relation_class(
                self.x_axis, [10.1, -5.231, 123.0, 0.0, 12.465, 5.0]
            )

            self.simple_relation = self.relation_class(
                self.x_axis, [10, 20, 30, 40, 50, 60]
            )
        ...
        def test_properties(self):
            self.assertIsInstance(self.relation.x, Axis)

            relation_array_like = self.relation_class([1, 2, 3], [1, 2, 3])
            self.assertIsInstance(relation_array_like.x, Axis)
            ...
            self.assertIsInstance(self.relation.sample, (int, float))
            ...
            assert_array_equal(self.relation.array, [5, 6, 7, 8, 9, 10])

            self.relation.size = 7
            self.assertEqual(self.relation.size, 7)
        ...
        def test_min(self):
            r = self.relation_class(
                [0.0, 0.1, 0.2, 0.3], [0.0, 2.123123, 10.123, -10.123]
            )
        ...
        def test_get_data(self):
            ...
            assert_array_almost_equal(
                y, [10.1, -5.231, 123.0, 0.0, 12.465, 5.0]
            )
        ...
        def test_differentiation(self):
            ...
            assert_array_equal(
                diff_x, self.x_axis.array[:-1] + self.x_axis.sample / 2
            )

            assert_array_equal(diff_y, [100, 100, 100, 100, 100])
        ...
        def test_integration(self):
            ...
            assert_array_equal(integer_x, self.x_axis.array[1:])

            assert_array_equal(integer_y, [1.5, 4.0, 7.5, 12.0, 17.5])

        def test_interpolate_extrapolate(self):

            new_x = Axis.get_using_end(0.0, 0.5, 0.05)
            ...
            new_x = Axis.get_using_end(-0.1, 0.6, 0.05)
        ...
        def test_equalize(self):
            ...
            new_relation.sample = 0.05
            new_simple_relation.sample = 0.05

            new_relation.end = 0.5
            new_simple_relation.end = 0.5
        ...
        def test_correlate(self):
            one_relation = self.relation_class(
                self.x_axis, np.zeros(self.x_axis.size)
            )
            one_relation.y[0] = 1.0
            ...
            assert_array_almost_equal(
                result.y, [0, 0, 0, 0, 0, 10, 20, 30, 40, 50, 60]
            )

            two_relation = self.relation_class(
                self.x_axis, np.zeros(self.x_axis.size)
            )
            two_relation.y[0] = 1.0
            two_relation.y[1] = 2.0
            result = self.relation_class.correlate(
                self.simple_relation, two_relation
            )

            self.assertIsInstance(result, self.relation_class)
            assert_array_almost_equal(
                result.y, [0, 0, 0, 0, 20, 50, 80, 110, 140, 170, 60]
            )
        ...
        def test_convolve(self):
            ...
            assert_array_almost_equal(
                result.y, [10, 20, 30, 40, 50, 60, 0, 0, 0, 0, 0]
            )

            self.assertIsInstance(result, self.relation_class)

        def test_math_operations(self):
            x = Axis.get_using_end(start=0, end=4, sample=1)
            ...
            for m in operation:
                for k in [(r2, y2), (2, 2)]:
                    with self.subTest(
                        f'Operation {m}, operation elements {k}', k=k, m=m
                    ):
        ...
        def _math_check(
            r1: Relation,
            ry: Union[Relation, Number],
            x: Axis,
            y1: np.array,
            y: Union[np.ndarray, Number],
            operation: str,
        ):
