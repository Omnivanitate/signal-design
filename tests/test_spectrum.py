...
from signal_design.spectrum import Spectrum
from signal_design.axis import Axis
from signal_design.exc import ConvertingError, TypeFuncError
from signal_design.relation import Relation
from signal_design.signal import Signal
...
class TestSpectrum(WrapperTestRelation.TestBaseRelation):
    def setUp(self) -> None:
        ...
        self.x_axis = Axis.get_using_end(start=0, end=0.5, sample=0.1)
        ...
        self.x_axis_2 = Axis.get_using_end(start=0, end=0.6, sample=0.1)
        self.simple_second_relation = self.relation_class(
            self.x_axis_2, [10, 20, 30, 40, 50, 60, 70]
        )
    ...
    def test_get_amplitude_spectrum(self):
        ...
        amp = spectrum.get_amplitude_spectrum()
    ...
    def test_get_spectrum_from_phase_and_amplitude(self):
        ...
        amplitude_spectrum = Relation([0, 1, 2, 3, 4, 5], [1, 2, 4, 6, 8, 10])
        phase_spectrum = Relation([0, 1, 2, 3, 4, 5], [1, 2, 4, 6, 8, 10])
        spectrum = self.relation_class.get_from_amplitude_phase(
    ...
    def test_add_phase(self):
        spectrum = self.relation_class([0, 1, 2, 3, 4, 5], [1, 2, 4, 6, 8, 10])
        phase_spectrum = self.relation_class(
            [0, 1, 2, 3, 4, 5], [1, 2, 4, 6, 8, 10]
        )
