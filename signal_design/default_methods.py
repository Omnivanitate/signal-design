...
def get_axis_from_array(
    ...
    Args:
        ...
        sample (Optional[RealNumber]): Default None. Sample of array.
            If sample is None, it well be calculated by method.
...
def one_integrate(relation: 'relation.Relation') -> float:
    """Integration.
...
def integrate(relation_data: 'relation.Relation') -> 'relation.Relation':
    """Integration.
...
def differentiate(relation_data: 'relation.Relation') -> 'relation.Relation':
    """Differentiation.
...
def correlate(
    ...
    x_axis = axis.Axis(start=-r1.end, sample=r1.sample, size=r1.size * 2 - 1)
...
def spectrum2signal(
    ...
    if time is None or isinstance(time, int):
        ...
        if time_start is None:
            time = axis.Axis(
                start=0.0, sample=dt, size=size, end=(amplitude.size - 1) * dt
            )
            ...
            time = axis.Axis(
                ...
                sample=dt,
                size=size,
                end=time_start + (amplitude.size - 1) * dt,
            )
